...
metrics. We evaluate the system on 160 streaming experiments across four
cluster topologies, three arrival rates, and four failure-injection regimes.
The MAPE-K controller adapts \(\varepsilon\) up to 14 times per stream
without exceeding a 4\% makespan penalty, while a tabular Q-learning agent
converges from 2.6\(\times\) to 1.6\(\times\) the HEFT-LC baseline within
350 episodes despite a coarse state encoding. We discuss the limits of
tabular RL for combinatorial scheduling and outline a deep-RL extension.
\end{abstract}
...
\begin{itemize}
  ...
  \item A MAPE-K autonomic controller \cite{kephart2003vision} that monitors
        the rolling load imbalance and adjusts \(\varepsilon\) online; we
        show it triggers up to 14 adaptations per stream without
        substantially harming makespan.
  ...
  \item An open-source streaming simulator with failure injection,
        reproducing all results from a single command.
\end{itemize}
...
We run 160 simulations covering four scenarios:
\begin{description}
...
\subsection{Autonomic controller (D)}
Across the 16 runs of scenario D the controller triggered a mean of 13.5
adaptations per stream on the \texttt{skewed\_8} cluster, with final
\(\varepsilon\) values ranging from 0.04 to 0.23 depending on workload mix.
Figure~\ref{fig:fig4} plots a representative adaptation trace --- the
controller raises \(\varepsilon\) early when the cluster has just been
populated and imbalance is high, then lowers it once the rolling window
falls below target. Mean makespan increases by 3.5\%, mean imbalance is
unchanged. The contribution is not a makespan win but the removal of the
manual tuning step: the controller arrives at workload-specific
\(\varepsilon\) values without operator intervention.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{../figures/fig4_adaptation_trace.pdf}
\caption{Online \(\varepsilon\) trace produced by the MAPE-K controller.}
\label{fig:fig4}
\end{figure}
...
\textbf{When the autonomic controller helps.} The controller's benefit is
not visible on workloads where the default \(\varepsilon=0.05\) is already
near-optimal --- this is the case for our stable Poisson stream. The
controller's value would emerge on non-stationary workloads (workload mix
shifting over time) or on operator deployments where the right
\(\varepsilon\) is unknown. We treat this as a limitation: the experiments
demonstrate that the loop closes and tracks the target imbalance, not
that it strictly improves makespan in steady state.
...
(iii) a tabular Q-learning agent. Experimental evidence is honest:
DLS clearly wins on heterogeneous-bandwidth clusters, the autonomic loop
closes without harming throughput, and the RL agent converges toward but
does not beat the strong baseline. Three follow-up directions stand out:
